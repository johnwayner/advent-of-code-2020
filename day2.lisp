(defpackage :com.johnwayner.aoc-20.day-2
  (:use :cl))

(in-package :com.johnwayner.aoc-20.day-2)
...
(defun parse-rule-and-pw (rule-and-pw-string)
  (let* ((rule-tokens (tokenize rule-and-pw-string))
         (range-string (nth 0 rule-tokens))
         (range-sep-index (position #\- range-string))
         (low (parse-integer (subseq range-string 0 range-sep-index)))
         (high (parse-integer (subseq range-string (1+ range-sep-index))))
         (char (elt (nth 1 rule-tokens) 0))
         (password (nth 2 rule-tokens)))
    (list low high char password)))

(defun rule-and-pw-valid-part-1-p (rule-and-pw-list)
  (<= (nth 0 rule-and-pw-list)
      (count (nth 2 rule-and-pw-list) (nth 3 rule-and-pw-list))
      (nth 1 rule-and-pw-list)))

(defun xor (a b)
  (equalp 1 (logxor (if a 1 0) (if b 1 0))))

(defun rule-and-pw-valid-part-2-p (rule-and-pw-list)
  (let ((password (nth 3 rule-and-pw-list))
        (char (nth 2 rule-and-pw-list))
        (pos-1 (- (nth 0 rule-and-pw-list) 1))
        (pos-2 (- (nth 1 rule-and-pw-list) 1)))
    (xor (and (< pos-1 (length password)) (equalp char (elt password pos-1)))
         (and (< pos-2 (length password)) (equalp char (elt password pos-2))))))

(defvar input-part-1 "13-15 c: cqbhncccjsncqcc
2-3 v: zvdvfd
...
(defun count-valid (validity-fn)
  (loop with lines = (make-string-input-stream input-part-1)
        for line = (read-line lines nil)
        while line
        when (funcall validity-fn (parse-rule-and-pw line))
          count 1))

(defun run-part-1 ()
  (count-valid #'rule-and-pw-valid-part-1-p))

(defun run-part-2 ()
  (count-valid #'rule-and-pw-valid-part-2-p))
