(defpackage :com.johnwayner.aoc-20.day-4
  (:use :cl))

(in-package :com.johnwayner.aoc-20.day-4)

(declaim (optimize (safety 3)))

(defclass credentials ()
  ((birth-year :initarg :byr :documentation "Birth Year" :accessor birth-year)
   (issue-year :initarg :iyr :documentation "Issue Year" :accessor issue-year)
   (expiration-year :initarg :eyr :documentation "Expiration Year" :accessor expiration-year)
   (height :initarg :hgt :documentation "Height" :accessor height)
   (hair-color :initarg :hcl :documentation "Hair Color" :accessor hair-color)
   (eye-color :initarg :ecl :documentation "Eye Color" :accessor eye-color)
   (passport-id :initarg :pid :documentation "Passport ID" :accessor passport-id)
   (country-id :initarg :cid :documentation "Country ID" :accessor country-id)
   (optional-fields :initform '(country-id) :allocation :class :accessor optional-fields)))
...
(defmethod valid-part-1-p ((obj credentials))
  (loop for slot in (mapcar #'sb-mop:slot-definition-name (sb-mop:class-slots (class-of obj)))
        unless (or (member slot (optional-fields obj))
                   (slot-boundp obj slot))
          return nil
        finally (return t)))


(defun height-valid-p (s)
  (let* ((unit-index (or (search "in" s)
                         (search "cm" s)))
         (unit (when unit-index (subseq s unit-index)))
         (magnitude (when unit-index (parse-integer s :end unit-index))))
    (when unit
      (if (equalp "in" unit)
          (<= 59 magnitude 76)
          (<= 150 magnitude 193)))))

(defun hair-color-valid-p (s)
  (and (= 7 (length s))
       (equalp #\# (elt s 0))
       (every (lambda (c) (or (digit-char-p c)
                              (char-not-greaterp #\a c #\f)))
              (subseq s 1))))

(defmethod valid-part-2-p ((obj credentials))
  (handler-case
      (and (<= 1920 (parse-integer (birth-year obj)) 2002)
           (<= 2010 (parse-integer (issue-year obj)) 2020)
           (<= 2020 (parse-integer (expiration-year obj)) 2030)
           (height-valid-p (height obj))
           (hair-color-valid-p (hair-color obj))
           (member (eye-color obj) '("amb" "blu" "brn" "gry" "grn" "hzl" "oth") :test #'equalp)
           (and (= 9 (length (passport-id obj)))
                (every #'digit-char-p (passport-id obj))))
    (t () nil)))

(defun tokenize (string &optional (separator #\ ))
  (loop with start-index = 0
        for c across string
        for index from 0
        when (equalp c separator)
        collect (subseq string start-index index)
        and do (setq start-index (1+ index))
        when (equalp index (- (length string) 1))
        collect (subseq string start-index (1+ index))))

(defun parse-attribute-line (line)
  (mapcan (lambda (x) (tokenize x #\:)) (tokenize line)))

(defun parse-input (input)
  (loop for line in (tokenize input #\Linefeed)
        with cred-attrs = nil
        if (> (length line) 0)
          do (setf cred-attrs (nconc cred-attrs (parse-attribute-line line)))
        else
          collect cred-attrs into creds
          and do (setf cred-attrs nil)
        finally
           (return (append creds (list cred-attrs)))))

(defun make-credentials (attr-list)
  (apply #'make-instance 'credentials
...
(defun parse-credentials (input)
  (mapcar #'make-credentials (parse-input input)))

(defvar *input-sample-part-1* "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
(defun run-part-2 ()
  (count-if #'valid-part-2-p (parse-credentials *input-part-1*)))
